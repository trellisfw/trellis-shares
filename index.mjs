...
async function newJob (job, { jobId, log, oada }) {
  ...
  info(
    `Successfully linked src = ${job.config.src} to dest = ${dest} with chroot ${chroot}`
  )
...
async function putLinkAndEnsureParent ({ path, data, chroot, tree, oada }) {
  ...
  if (exists === 404) {
    ...
    const newid = await oada
      ...
      .then(r => r.headers['content-location'].replace(/^\//, ''))
    trace(
      `#putLinkAndEnsureParent: posted new resource to /resources for the child, newid = ${newid}`
    )
