...
describe('success job', () => {
  ...
  before(async function() {
    ...
    await con.put( { path: `/bookmarks/trellisfw/cois`, tree, data: {
      [coikey]: { _id: `resources/${coikey}`, _rev: 0 },
    }});

    console.log('Before: getting first trading partner to use as guinea pig');
    const tp = await con.get({ path: '/bookmarks/trellisfw/trading-partners' })
      .then(r=>_.filter(_.keys(r.data), k => !k.match(/^_/))[0])
      .then(tpid => con.get({ path: `/bookmarks/trellisfw/trading-partners/${tpid}` }))
      .then(r=>r.data);
  ...
  it('should put the job under today\'s day-index within jobs-success', async() => {
    ...
    const result = await con.get({ path: `/bookmarks/services/trellis-shares/jobs-success/day-index/${day}/${jobkey}` }).then(r=>r.data);
